...
### ⚠ 빠뜨렸던 것 — Auth 설정 (2026-08-21 · 실제 화면 확인으로 드러남)

**프로젝트 이전은 DB 만 옮기는 일이 아니었다.** 마이그레이션·데이터·env·백업을 다
맞추고도 프로덕션 로그인이 안 됐다. ingreed 프로젝트는 Auth 를 쓴 적이 없어
`site_url` 이 `http://localhost:3000`, `uri_allow_list` 가 빈 값이었다. 옛
프로젝트에 있던 설정은 pg_dump 에도 마이그레이션에도 담기지 않는다.

고친 뒤 두 번째 증상이 나왔다 — 설정 변경으로 Auth 서버가 재시작되며 그 직후
발급된 토큰이 `PGRST303 JWT issued at future` 로 거부됐다. 시각 차이를 재보니
DB·로컬·토큰이 모두 1초 안이었고, 1분 뒤 재로그인으로 풀렸다.

`disable_signup` 도 기본값이 **열려 있었다**. 오너 1인 앱이라 잠갔다.

### ✅ 실제 화면으로 확인 (2026-08-21)

브라우저로 로그인해 눈으로 봤다. 테스트가 통과해도 화면은 깨질 수 있다.

| 확인 | 결과 |
|---|---|
| 복원 데이터 | 체중 85.2kg · 수면 7.1시간 · **"마지막 식사 후 972시간"**(= 7/12, 복원된 마지막 기록과 일치) |
| '오늘' 화면 | 등급 대신 "아직 진행 중" — §2.6 설계대로 |
| **ingreed 제품 검색** | 김치사발면 → **80g · 354kcal · 나트륨 1414mg** — 8/20 실측값과 일치. env 가 오늘 처음 붙었으니 이것이 첫 실왕복이다 |
