...
G4b-4 pg_cron으로 등록한 단식 리마인더가 실제 발화 — ⬜ 이월(아래 각주)
```

> **G4b-4 이월 사유(2026-07-28, 오너 승인)**: 착수 중 조사 결과 Swift 원본에도
> 단식 리마인더의 실제 알림 스케줄링 코드(iOS 로컬 알림 등)가 없다 —
> `fastingStatus()`는 호출 시점에 순수 계산만 하는 폴링 방식이고, 예약된
> 알림을 발사하는 코드 자체가 없다. 방향성 문서(§5.2.2 부근)가 웹 이관 시
> "Web Push"를 가정했지만, 정작 이 문서 §P5(웹 UI)가 "Web Push는 P5 범위
> 밖입니다. 하지 마세요"라고 명시적으로 금지하고 있어 G4b-4를 문자 그대로
> 만족시키려면 새 외부 알림 채널(이메일 발송 등) 도입이 필요했다 — 이는
> CLAUDE.md 안전 바닥(외부 API 신규는 사전 승인) 대상이라 이번 P4b 세션의
> 스코프 밖으로 이월한다. pg_cron 자체도 이번 세션엔 도입하지 않았다(실제
> 발송 채널이 결정돼야 스케줄 대상이 의미가 있음). 실제 알림 채널(이메일/
> Web Push 예외 허용/기타) 결정은 오너 몫으로 남기고, 결정 후 별도 phase로
> 진행한다.
