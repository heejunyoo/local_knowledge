...
|----|-----------|----------|-----------|-----|-----|-----|------|
| **F-S1** | **실녹음 dogfood 게이트 패키지** (체크리스트+자동 품질 스냅샷) | J2 | 메모리 제품은 “캡처 신뢰”가 핵심 | 5 | 5 | S | 기능보다 **검증 인프라** |
| **F-S2** | **요약 품질 1단계 업** (구조화 cloud refine: 결정/액션만 선택적 LLM, 한 줄 강제) | J2 | RAG/KB 품질 먼저 원칙 | 5 | 5 | M | extractive 상한 돌파 |
| **F-S3** | **요약↔전사 타임스탬프 점프** | J2 | 리뷰 UX 표준 | 5 | 4 | S–M | 열람 완성도 |
| **F-S4** | **ASR 필드 고정** (whisper 설치 스크립트 **또는** Speech-only 명시+배지) | J2 | 도구 신뢰 | 5 | 5 | S | verify-tools FAIL 해소 |
