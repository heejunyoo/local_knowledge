...
| 파일 | 종류 | 처리 |
|---|---|---|
| `CyberSourceKey_20250918223005.pem` | RSA/EC 개인키로 추정 | **P0-5에서 `~/Secrets/`로 격리** |
| `postmanv2.html` | HTML (Postman 내보내기 추정) | P0-5에서 내용 확인 후 처리 |
| `90 ⚙️ 첨부파일/*.png`, `*.gif` (30개) | 이미지 | 그대로 유지 (스크린샷/프리뷰) |
| `90 ⚙️ 첨부파일/Heejun_PaySlip_2410_2503.pdf` | **급여명세서 (개인 금융정보)** | P0-5 스캔 시 별도 취급 검토 |
| `90 ⚙️ 첨부파일/유희준_재직증명서(유이현).pdf` | **재직증명서 (개인정보)** | P0-5 스캔 시 별도 취급 검토 |
| `90 ⚙️ 첨부파일/AD_4nX*.png` (2개) | 이미지(외부 저장 서비스 자동 명명) | 그대로 유지 |
| `.DS_Store` | macOS 메타 | 무시 대상 |

> **추가 발견 (액션플랜 C-5에 없던 항목)**: 급여명세서·재직증명서 PDF 2건은 "비밀정보"(API 키 등)는 아니지만 **개인 식별/금융 정보**이므로, P2에서 vault를 Git 프라이빗 레포에 올릴 때 동일 수준의 주의가 필요하다. 값을 문서에 옮기지 않았고, 처리는 P0-5에서 결정한다.
